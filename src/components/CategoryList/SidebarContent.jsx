...
const SidebarContent = () => {
    ...
    return (
        <List>
                    ...
                    <ListItemButton onClick={() => handleCategoryClick(category.name)}>
                        <ListItemText
                            primaryTypographyProps={{
                                style: {
                                    ...subtitleStyle,
                                    color: selectedCategory === category.name ? "blue" : "inherit"
                                }
                            }}
                                ...
                                >
                                    <ListItemText
                                        primaryTypographyProps={{
                                            style: {
                                                ...childrenStyle,
                                                color: selectedSubcategory === subcategory ? "blue" : "inherit" 
                                            }
                                        }}
                                        primary={subcategory}
                                    />
            ...
            <div style={{ width: "max-content", marginBlock: "1.5rem", display: "flex", flexDirection: "column" }}>
                <Typography variant="body2" style={subtitleRange}>Calificación</Typography>
                {[4, 3, 2, 1].map((stars) => (
                    <div style={{ display: "flex", alignItems: "center", width: "100%", marginTop: 6 }}>
                        <FormControlLabel
                            ...
                            label={
                                <div style={{ display: "flex", textAlign: "center", fontSize: isMobile ? "12px" : "0.9rem", gap: "4px", width: isMobile ? "100%" : "100%" }}>
                                    <Rating readOnly name="size-small" sx={{ m: 0, ml: 1, p: 0, mb: 1 }} value={stars} size="small" />
                                    <p>
                                        Y más
                                    </p>
            ...
            {/* Rango de precios */}
            <div style={{ width: "max-content", maxWidth: isMobile ? "max-content" : "12vw" }}>
                <Typography variant="body2">
                    Rango de precios
                </Typography>
                {priceRange.map((range) => (
                    <ListItemButton key={range} onClick={() => console.log("Rango de precio seleccionado:", range)}>
                        <ListItemText primaryTypographyProps={{ style: childrenFilter }} primary={range} />
                    </ListItemButton>
                ))}
                <div style={{ display: "flex", gap: 4, width: isMobile ? "80%" : "100%" }}>
            ...
            {/* Marcas */}
            <div style={{ display: "flex", flexDirection: "column", width: "max-content", marginTop: "1.5rem" }}>
                <Typography variant="body2" style={subtitleRange}>Marcas</Typography>
                {checkboxFilters.marcas.map((marca) => (
                    <div key={marca} style={{ display: "flex", alignItems: "center", textAlign: "center", marginBottom: "5px" }}>
                        <div
                            style={{
                                width: "14px",
                                height: "14px",
                                ...
                                borderRadius: "3px",
                                marginRight: "5px",
                                backgroundColor: selectedMarcas.indexOf(marca) !== -1 ? "#1B1AFF" : "transparent",
                                cursor: "pointer"
                            }}
